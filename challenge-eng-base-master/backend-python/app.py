...
def search_movies():
    ...
    where_clauses = []
    from_clause = 'movies m join ratings r on m.movieId=r.movieId join links l on l.movieId=m.movieId'
    ...
    query = 'SELECT m.title, m.genres, AVG(r.rating), l.imdbId, l.tmdbId FROM {}{} GROUP BY' \
            ' m.movieId LIMIT {} OFFSET {}'.format(from_clause, where_clause,
                                                   limit, (page-1)*limit)
    ...
    with db.cursor() as cur:
        ...
        return flask.jsonify(
            {'data':
                 [[title, genres.strip().split('|'), float(avg_rating),
                   'www.imdb.com/title/{}'.format(imdb_id),
                   'www.themoviedb.org/movie/{}'.format(tmdb_id)]
